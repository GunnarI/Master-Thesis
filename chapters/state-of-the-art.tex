...
% From CGM1.0 to CGM2.1 the medial knee marker for knee joint center calibration.
% Furthermore, hip joint center calibration calculations are improved and pelvic angles defined to 
...
% It also fixes
One improvement worth mentioning, when looking at CGM2.2 \cite{cgm22}, is the change from how the model is kinematically fitted to the motion capture. 
CGM2.2 introduces the use of \ac{IK} and with it some improvements that can be read about on the CGM2 website. 
CGM2.3 then introduces additional markers to minimize a soft tissue artifact accompanying the method when using the thigh and calf wands.
\begin{figure}[t]
    \centering
    \includegraphics[width=0.7\textwidth]{img/CGM23_markerset_highlight.png}
    \caption{The markerset as defined for version CGM2.3. The changes from the original CGM1.0 markerset are highlighted with orange circles/arrows. Adapted from pyCGM2 official website \cite{Leboeuf2019}}
    \label{fig:cgm23-markerset-guidelines}
\end{figure}
