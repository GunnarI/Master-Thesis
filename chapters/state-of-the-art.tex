...
Figure x demonstrates the flow of information through the \ac{LSTM} and equations \ref{eq:lstm_sigmoid_generic}--\ref{eq:last_lstm_step} write out the process.
...
Equations \ref{eq:ann-node} become:
\begin{equation}
...
The flow of data through the cell is then as shown in equations \ref{eq:lstm-ff}:
\begin{subequations}
\label{eq:lstm-ff}
\begin{align}
    f^{(t)} &= \sigma^f\\
...
\end{align}
\end{subequations}
Note here that the variable $s^{(t-1)}$ is the state of the \ac{LSTM} cell from last time step, and serves here as an input into the cell at current state.
This is additionally to the inputs into the sigmoid function, namely the input features and previous hidden state.
