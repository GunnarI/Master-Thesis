...
The kinematic and kinetic outputs are based on the orientation of the distal segment relative to the proximal segment. 
For the knee joint the origin of the coordinate system is located at the knee joint center, with the z axis pointing upwards in the coronal plane of the femur and the y axis pointing left (i.e. medial on right knee but lateral on left knee) in the coronal plane.
...
The package is an upgrade to the \ac{PiG} model with the first version intended to replicate the \ac{PiG} model outputs exactly and following versions to build upon it, fixing known issues \cite{Leboeuf2019}.
