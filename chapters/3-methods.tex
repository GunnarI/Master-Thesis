...
Data was collected on x subjects with anthropometric data measured and some variation during trials as explained in table \ref{tab:subject-table}.
...
Finally, the \ac{EMG} signal is downsampled to match the frequency of the model outputs which is $100Hz$. The downsampling is done simply by selecting one random sample from each $10ms$. The downsampling is only necessary for the training, not for running the \ac{NN} in real-time.
