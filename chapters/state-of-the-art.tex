...
\textit{\Acl{ID}} is, as defined by the Dictionary of Sport and Exercise Science and Medicine \cite{inversedynamics}, "the calculation of forces and moments by using kinematics and data for mass and moment of inertia, effectively using Newton's second law of motion ($f = ma$) in reverse."
...
The software offers the option of running biomechanical models on the data captured, including scaling the labelling skeleton based on anthropometric data, performing inverse kinematics.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[t]{0.3\textwidth}
        ...
        \label{fig:cgm23-markerset-experiment-capture}
    \end{subfigure}
    \caption{Knee joint center placement and coordinate system setup. The red, green, and blue (RGB) color code indicates X-, Y-, and Z-axis respectively. (a) The femur for the left leg. The faded yellow points show the hip and knee joint center on the femur while the solid yellow are the lateral epicondyle and lateral thigh markers. The knee center is defined by a right angle triangle formed by the hip joint center, epicondyle marker and the knee joint center in the coronal plane, indicated by the red shadow. (b) Grey circle shows the sagittal plane of the femur. The flexion/extension angle is defined by the rotation of the fibia around the y-axis in the sagittal plane, indicated by the green angle. (c) Model built from CGM2.3 setup but the orientation knee joint coordinates system is the same. Note that in the right leg, the y-axis points towards the medial epicondyle unlike in the left leg where it points towards the lateral epicondyle.}
...
The package is an upgrade to the \ac{PiG} model with the first version intended to replicate the \ac{PiG} model outputs exactly and following versions to build upon it, fixing known issues.
One improvement worth mentioning, when looking at CGM2.2 \cite{cgm22}, is the change from how the model is kinematically fitted to the motion capture. CGM2.2 introduces the use of \ac{IK} and with it some improvements that can be read about on the CGM2 website. CGM2.3 then introduces additional markers to minimize a soft tissue artifact accompanying the method when using the thigh and calf wands.
